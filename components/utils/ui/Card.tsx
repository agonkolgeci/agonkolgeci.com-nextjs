...
export function Card({ orientation = Orientation.VERTICAL, animation = Animation.NONE, children } : { orientation?: Orientation, animation?: Animation, children: React.ReactNode }): React.ReactNode {
    return (
        <li className={`flex flex-col justify-between lg:${orientation} w-full h-full bg-secondary border border-white/8 text-white shadow-2xl rounded-2xl overflow-hidden transition-all duration-300 hover:border-white/20 ${animation}`}>
            {children}
        </li>
    )
}
